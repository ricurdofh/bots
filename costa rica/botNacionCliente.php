...
set_time_limit (90);
    ...
    $BOT_BROWSER_COOKIES = dirname(__FILE__).'/BotNacion.txt';
    for ($w=1; $w<7; $w++)
    {
        if ($w > 5)
            exit ('No pude cargar la URL : ' . $URL . ' puede ser un cambio en el diseño o errores de servidor ' . $htmlResultPage);
            
        $htmlResultPage = EjecutaCurl($URL, $agentBrowser, $BOT_BROWSER_COOKIES, 'http://www.nacion.com/');
        
        $pattern = '/\<div class\=\"md-news\-latest md sk\-md1\"\>(.*?)\<\/div\>\<\!\-\- \.md\-news\-latest\.md\.sk\-md1 \-\-\>/si';
    ...
    foreach ($urlNoticias[0] as $k=>$urlNoticia) {
        if ($cont > $max)
            break;
        $urlNoticia = 'http://www.nacion.com' . $urlNoticia;
    ...
    function agregaNoticia($urlNoticia, $categoriaNoticia, $agentBrowser, $BOT_BROWSER_COOKIES, $URL)
    {
        ...
        $patron = '/\<\/div\>\<\!\-\- \.md\-news\-latest\.md\.sk\-md1 \-\-\>(.*?)\<\/div\>\<\!\-\- \.mce\-body \-\-\>/si';
        ...
        $patron = '/\<link rel\=\"alternate\" title\=\"(.*?)\" href\=\"/si';
