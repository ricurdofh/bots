...
set_time_limit (90);
    ...
    $URL = 'http://everardoherrera.com/';
    $BOT_BROWSER_COOKIES = dirname(__FILE__).'/BotAlPieDelDeporte.txt';
    for ($w=1; $w<7; $w++)
    {
        if ($w > 5)
            exit ('No pude cargar la URL : ' . $URL . ' puede ser un cambio en el diseño o errores de servidor ' . $htmlResultPage);
            
        $htmlResultPage = EjecutaCurl($URL, $agentBrowser, $BOT_BROWSER_COOKIES, 'http://everardoherrera.com/');
        
        $pattern = '/\<div id\=\"recientes\"\>(.*?)\<div id\=\"EquipoTablaRedactores\"\>/si';
    ...
    foreach ($urlNoticias[0] as $k=>$urlNoticia) {
        if ($cont > $max)
            break;
        $urlNoticia = 'http://everardoherrera.com' . $urlNoticia;
    ...
    function agregaNoticia($urlNoticia, $categoriaNoticia, $agentBrowser, $BOT_BROWSER_COOKIES, $URL)
    {
        ...
        $patron = '/\<span itemprop\=\"articleBody\"\>(.*?)\<div class\=\"moduletable\"\>/si';
        ...
        $patron = '/\<li class\=\"leaf active\-trail\"\>\<a href\=\"\/(.*?)\"/si';
        ...
        $tituloNoticia = str_replace('| Repretel', '', trim(htmlspecialchars_decode($tituloNoticia[0])));
        ...
        $contenidoNoticia = preg_replace('/\<p\>\<img src\=\"(.*?)\<\/p\>/si', '', $contenidoNoticia[1]);
