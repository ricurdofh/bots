...
set_time_limit (90);
    ...
    $URL = 'http://www.teletica.com/';
    $BOT_BROWSER_COOKIES = dirname(__FILE__).'/BotTeletica.txt';
    for ($w=1; $w<7; $w++)
    {
        if ($w > 5)
            exit ('No pude cargar la URL : ' . $URL . ' puede ser un cambio en el diseño o errores de servidor ' . $htmlResultPage);
            
        $htmlResultPage = EjecutaCurl($URL, $agentBrowser, $BOT_BROWSER_COOKIES, 'http://www.teletica.com/');
        
        $pattern = '/\<div id\=\"NotasDestacadas\"\>(.*?)\<\/div\> \<\!\-\- Modul Entretenimiento \-\-\>/si';
    ...
    foreach ($urlNoticias[0] as $k=>$urlNoticia) {
        if ($cont > $max)
            break;
        // $urlNoticia = 'http://www.teletica.com' . $urlNoticia;
    ...
    function agregaNoticia($urlNoticia, $categoriaNoticia, $agentBrowser, $BOT_BROWSER_COOKIES, $URL)
    {
        ...
        $patron = '/\<div id\=\"Txt\" class\=\"resizeTxt\"\>(.*?)\<div id\=\"Actions\"\>/si';
        ...
        $patron = '/\<div class\=\"TitleTxt\"\>(.*?)\<\/p\>/si';
        ...
        $tituloNoticia = str_replace('| Teletica', '', trim(htmlspecialchars_decode($tituloNoticia[0])));
        ...
        if ($imagenes) {
            ...
            $img = $rutArchivo . '/' .  $nombFile . '.jpg';
             ...
             $ch = curl_init("http://www.nacion.com" . $imagenes[0]);
             $fp = fopen($img, 'wb');
    ...
    {
        $patron = '/NACIÓN|NACION|Nacion|POL&Iacute;TICA|La naci|LA NACI|La Naci|Editorial|Opini|Sucesos|SUCESOS|OPINION|Educaci|EDUCACI|En Campa|EN CAMPA|Ambiente|AMBIENTE|Poder|Popular|Poder Popular|Política|Politica|Gesti|Social|Gestión|Regiones|Region|educaci/';
