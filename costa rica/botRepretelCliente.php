...
set_time_limit (90);
    ...
    $URL = 'http://www.repretel.com/noticias';
    $BOT_BROWSER_COOKIES = dirname(__FILE__).'/BotRepretel.txt';
    for ($w=1; $w<7; $w++)
    {
        if ($w > 5)
            exit ('No pude cargar la URL : ' . $URL . ' puede ser un cambio en el diseño o errores de servidor ' . $htmlResultPage);
            
        $htmlResultPage = EjecutaCurl($URL, $agentBrowser, $BOT_BROWSER_COOKIES, 'http://www.repretel.com/');
        
        $pattern = '/\<div id\=\"fragment\-1\"(.*?)\<script type\=\"text\/javascript\"\>/si';
    ...
    $patron = '/(?<=\<div class\=\"title\"\>\<a href\=\")[^\"]+/si';
    ...
    foreach ($urlNoticias[0] as $k=>$urlNoticia) {
        if ($cont > $max)
            break;
        $urlNoticia = 'http://www.repretel.com' . $urlNoticia;
    ...
    function agregaNoticia($urlNoticia, $categoriaNoticia, $agentBrowser, $BOT_BROWSER_COOKIES, $URL)
    {
        ...
        $patron = '/\<div class\=\"field field\-body\"\>(.*?)\<div class\=\"field\-label\-inline\-first\"\>ETIQUETAS\:/si';
        ...
        // $pattern = '/\<ul class\=\"rslides\" id\=\"slider4\"\>(.*?)\<\/ul\>/si';
