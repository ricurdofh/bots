...
set_time_limit (90);
    ...
    $URL = 'http://diarioextra.com/';
    $BOT_BROWSER_COOKIES = dirname(__FILE__).'/BotDiarioExtra.txt';
    for ($w=1; $w<7; $w++)
    {
        if ($w > 5)
            exit ('No pude cargar la URL : ' . $URL . ' puede ser un cambio en el diseño o errores de servidor ' . $htmlResultPage);
            
        $htmlResultPage = EjecutaCurl($URL, $agentBrowser, $BOT_BROWSER_COOKIES, 'http://diarioextra.com/');
        
        $pattern = '/\<div class\=\"panes\"\>(.*?)\<\!\-\-  FIN \-\-\>/si';
    ...
    foreach ($urlNoticias[0] as $k=>$urlNoticia) {
        if ($cont > $max)
            break;
        $urlNoticia = 'http://diarioextra.com' . $urlNoticia;
    ...
    function agregaNoticia($urlNoticia, $categoriaNoticia, $agentBrowser, $BOT_BROWSER_COOKIES, $URL)
    {
        ...
        $patron = '/\<div class\=\"span\_12\_of\_12 txtContainer\"\>(.*?)\<\!\-\- creditos \-\-\>/si';
        ...
        $patron = '/\<div class\=\"categoria boxRed\"\>(.*?)\<\/div\>/si';
        ...
        $tituloNoticia = str_replace('Diario Extra - ', '', trim(htmlspecialchars_decode($tituloNoticia[0])));
    ...
    {
        $patron = '/NACIÓN|NACION|nacion|Nacion|POL&Iacute;TICA|La naci|LA NACI|La Naci|Editorial|Opini|Sucesos|SUCESOS|OPINION|Educaci|EDUCACI|En Campa|EN CAMPA|Ambiente|AMBIENTE|Poder|Popular|Poder Popular|Política|Politica|Gesti|Social|Gestión|Regiones|Region|educaci/';
