...
set_time_limit (90);
    ...
    $URL = 'http://www.elfinancierocr.com/';
    $BOT_BROWSER_COOKIES = dirname(__FILE__).'/BotElFinanciero.txt';
    for ($w=1; $w<7; $w++)
    {
        if ($w > 5)
            exit ('No pude cargar la URL : ' . $URL . ' puede ser un cambio en el diseño o errores de servidor ' . $htmlResultPage);
            
        $htmlResultPage = EjecutaCurl($URL, $agentBrowser, $BOT_BROWSER_COOKIES, 'http://www.elfinancierocr.com/');
        
        $pattern = '/\<div class\=\"gdu u1\-2\-c2 u\-last\"\>(.*?)\<\!\-\- \.gdu\.u1\-2\-c2\.u\-last \-\-\>/si';
    ...
    $patron = '/(?<=\<a class\=\"goto\-detail\" href\=\")[^\"]+/si';
    ...
    foreach ($urlNoticias[0] as $k=>$urlNoticia) {
        ...
        $urlNoticia = 'http://www.elfinancierocr.com' . $urlNoticia;
    ...
    function agregaNoticia($urlNoticia, $categoriaNoticia, $agentBrowser, $BOT_BROWSER_COOKIES, $URL)
    {
        ...
        $patron = '/\<div class\=\"pg-story\-body mce\"\>(.*?)\<\!\-\- \.pg\-story\-body \-\-\>/si';
        ...
        $patron = '/\<link rel\=\"alternate\" title\=\"(.*?)\"/si';
        preg_match($patron, $htmlResultPage, $categoriaNoticia);

        $categoriaNoticia = asignaCategoria($categoriaNoticia[1]);
        // var_dump($categoriaNoticia);
        // exit;
        
        // $categoriaNoticia = 1;
        
        $tituloNoticia = str_replace('- El Financiero', '', trim(htmlspecialchars_decode($tituloNoticia[0])));
        ...
        if ($imagenes) {
            ...
            $img = $rutArchivo . '/' .  $nombFile . '.jpg';
             ...
             $ch = curl_init("http://www.elfinancierocr.com" . $imagenes[0]);
             $fp = fopen($img, 'wb');
    ...
    {
        $patron = '/NACIÓN|NACION|nacion|Nacion|POL&Iacute;TICA|La naci|LA NACI|La Naci|Editorial|Opini|sucesos|Sucesos|SUCESOS|OPINION|Educaci|EDUCACI|En Campa|EN CAMPA|Ambiente|AMBIENTE|Poder|Popular|Poder Popular|Política|Politica|Gesti|Social|Gestión|Regiones|Region|educaci/';
        preg_match($patron, $stringCategoria, $categoria);
        if($categoria)
            $categoria = '1';
        else 
        {
            $patron = '/econom|Econom|ECONOM|Finanzas|Pymes/';
            preg_match($patron, $stringCategoria, $categoria);
            if($categoria)
                $categoria = '3';
            else
            {
                ...
                {
                    $patron = '/deportes|Deportes|DEPORTES|Legionarios/';
                    preg_match($patron, $stringCategoria, $categoria);
                    if($categoria)
                        $categoria = '5';
                    else
