...
set_time_limit (90);
    ...
    $URL = 'http://www.crhoy.com/';
    $BOT_BROWSER_COOKIES = dirname(__FILE__).'/BotCRHoy.txt';
    for ($w=1; $w<7; $w++)
    {
        if ($w > 5)
            exit ('No pude cargar la URL : ' . $URL . ' puede ser un cambio en el diseño o errores de servidor ' . $htmlResultPage);
            
        $htmlResultPage = EjecutaCurl($URL, $agentBrowser, $BOT_BROWSER_COOKIES, 'http://www.crhoy.com/');
        
        $pattern = '/\<div class\=\"ultimas\_noticias\"\>(.*?)\<\!\-\- Termina Noticias Recientes \-\-\>/si';
    ...
    foreach ($urlNoticias[0] as $k=>$urlNoticia) {
        if ($cont > $max)
            break;
        // $urlNoticia = 'http://www.crhoy.com' . $urlNoticia;
