...
set_time_limit (90);
    ...
    $URL = 'http://www.mep.go.cr/noticias';
    $BOT_BROWSER_COOKIES = dirname(__FILE__).'/BotMep.txt';
    for ($w=1; $w<7; $w++)
    {
        if ($w > 5)
            exit ('No pude cargar la URL : ' . $URL . ' puede ser un cambio en el diseño o errores de servidor ' . $htmlResultPage);
            
        $htmlResultPage = EjecutaCurl($URL, $agentBrowser, $BOT_BROWSER_COOKIES, 'http://www.mep.go.cr/');
        
        $pattern = '/\<div   class \= \"article\-others\"   \>(.*?)\<div class\=\"item\-list\"\>/si';
    ...
    foreach ($urlNoticias[0] as $k=>$urlNoticia) {
        ...
        $urlNoticia = 'http://www.mep.go.cr' . $urlNoticia;
    ...
    function agregaNoticia($urlNoticia, $categoriaNoticia, $agentBrowser, $BOT_BROWSER_COOKIES, $URL)
    {
        ...
        $patron = '/\<div class\=\"news\_content\"\>(.*?)\<\!\-\- Imagenes \-\-\>/si';
        ...
        $pattern = '/\<div class\=\"imagenContainer\"\>(.*?)\<\/div\>/si';
