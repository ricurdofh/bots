...
set_time_limit (90);
    ...
    $URL = 'http://prensalibre.cr/';
    $BOT_BROWSER_COOKIES = dirname(__FILE__).'/BotPrensaLibre.txt';
    for ($w=1; $w<7; $w++)
    {
        if ($w > 5)
            exit ('No pude cargar la URL : ' . $URL . ' puede ser un cambio en el diseño o errores de servidor ' . $htmlResultPage);
            
        $htmlResultPage = EjecutaCurl($URL, $agentBrowser, $BOT_BROWSER_COOKIES, 'http://prensalibre.cr/');
        
        $pattern = '/\<div id\=\"wrapCostaRica\"\>(.*?)\<div id\=\"boxPublicidad\-01\"\>/si';
    ...
    foreach ($urlNoticias[0] as $k=>$urlNoticia) {
        if ($cont > $max)
            break;
        $urlNoticia = 'http://prensalibre.cr' . $urlNoticia;
    ...
    function agregaNoticia($urlNoticia, $categoriaNoticia, $agentBrowser, $BOT_BROWSER_COOKIES, $URL)
    {
        ...
        $patron = '/\<div id\=\"texto\"\>(.*?)\<\!\-\- btn regresar \-\-\>/si';
        ...
        $patron = '/itemprop\=\"articleSection\"\>\<h1\>(.*?)\<\/h1\>/si';
        ...
        $pattern = '/\<div id\=\"imgDetalle\"\>(.*?)\<\/div\>/si';
        ...
        if ($imagenes) {
            ...
            $img = $rutArchivo . '/' .  $nombFile . '.jpg';
             ...
             $ch = curl_init("http://prensalibre.cr" . $imagenes[0]);
             $fp = fopen($img, 'wb');
