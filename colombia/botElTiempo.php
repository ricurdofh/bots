...
set_time_limit (90);
	...
	$URL = 'http://www.eltiempo.com/ultimas-noticias';
	$BOT_BROWSER_COOKIES = dirname(__FILE__).'/BotElTiempo.txt';
	for ($w=1; $w<7; $w++)
	{
		if ($w > 5)
			exit ('No pude cargar la URL : ' . $URL . ' puede ser un cambio en el diseño o errores de servidor ' . $htmlResultPage);
			
		$htmlResultPage = EjecutaCurl($URL, $agentBrowser, $BOT_BROWSER_COOKIES, 'http://www.eltiempo.com/ultimas-noticias');
		
		$pattern = '/id\=\"inicial\"\>(.*?)\id\=\"mas\_noticias\_col3\"\>/si';
	...
	$patron = '/(?<=\<h2 itemprop\=\"name\"\>\<a href\=\")[^\"]+/si';
	...
	foreach ($urlNoticias[0] as $k=>$urlNoticia) {
		if ($cont > $max)
			break;
		$urlNoticia = 'http://www.eltiempo.com' . $urlNoticia;
