...
set_time_limit (90);
    ...
    $URL = 'http://panamaamerica.com.pa/';
    $BOT_BROWSER_COOKIES = dirname(__FILE__).'/BotPanamaAmerica.txt';
    for ($w=1; $w<7; $w++)
    {
        if ($w > 5)
            exit ('No pude cargar la URL : ' . $URL . ' puede ser un cambio en el diseño o errores de servidor ' . $htmlResultPage);
            
        $htmlResultPage = EjecutaCurl($URL, $agentBrowser, $BOT_BROWSER_COOKIES, 'http://panamaamerica.com.pa/');
        
        $pattern = '/\<div class\=\"panel\-pane pane\-views pane\-noticia\-1\" \>(.*?)id\=\"masleidascuadros\"\>/si';
    ...
    $patron = '/(?<=\<div class\=\"imagenmundo\"\>\<a href\=\")[^\"]+/si';
    ...
    foreach ($urlNoticias[0] as $k=>$urlNoticia) {
        if ($cont > $max)
            break;
        $urlNoticia = 'http://panamaamerica.com.pa' . $urlNoticia;
    ...
    function agregaNoticia($urlNoticia, $categoriaNoticia, $agentBrowser, $BOT_BROWSER_COOKIES, $URL)
    {
        ...
        $patron = '/\<div class\=\"field field\-name\-body(.*?)\<div class\=\"clearfix\"\>/si';
        ...
        $tituloNoticia = str_replace('| Panamá América', '', trim(htmlspecialchars_decode($tituloNoticia[0])));
        ...
        $contenidoNoticia = strip_tags($contenidoNoticia[0], '<br></br></ br><iframe></iframe><p></p><object></object><param></param>');
        ...
        $pattern = '/\<img class\=\"field\-slideshow\-image(.*?)\/\>/si';
    ...
    {
        $patron = '/NACIÓN|NACION|Nacion|nacion|Nación|nación|POL&Iacute;TICA|La naci|LA NACI|La Naci|Editorial|Opini|Sucesos|SUCESOS|OPINION|Educaci|EDUCACI|En Campa|EN CAMPA|Ambiente|AMBIENTE|Poder|Popular|Poder Popular|Política|Politica|Gesti|Social|Gestión|Regiones|Region|panama|Panama|PANAMA/';
        ...
        else 
        {
            ...
            {
                $patron = '/EUROPA|internacional|mundo|Mundo|MUNDO|Internacional/';
                preg_match($patron, $stringCategoria, $categoria);
                if($categoria)
                    $categoria = '4';
                else
