...
set_time_limit (90);
    ...
    $URL = 'http://www.telemetro.com/';
    $BOT_BROWSER_COOKIES = dirname(__FILE__).'/BotTelemetro.txt';
    for ($w=1; $w<7; $w++)
    {
        if ($w > 5)
            exit ('No pude cargar la URL : ' . $URL . ' puede ser un cambio en el diseño o errores de servidor ' . $htmlResultPage);
            
        $htmlResultPage = EjecutaCurl($URL, $agentBrowser, $BOT_BROWSER_COOKIES, 'http://www.telemetro.com/');
        
        $pattern = '/\<h2 class\=\"headline\"\>Las ÚLTIMAS\<\/h2\>(.*?)\<\/div\>\<\!\-\- \.md\-agenda \-\-\>/si';
    ...
    foreach ($urlNoticias[0] as $k=>$urlNoticia) {
        if ($cont > $max)
            break;
        $urlNoticia = 'http://www.telemetro.com' . $urlNoticia;
    ...
    function agregaNoticia($urlNoticia, $categoriaNoticia, $agentBrowser, $BOT_BROWSER_COOKIES, $URL)
    {
        ...
        $patron = '/\<div class\=\"mce\-body cf\"\>(.*?)\<\/div\>\<\!\-\- \.mce\-body \-\-\>/si';
        ...
        $pattern = '/\<div class\=\"scr\-container main\-image\"\>(.*?)\<\/div\>\<\!\-\- \.scr\-container \-\-\>/si';
    ...
    {
        $patron = '/NACIÓN|NACION|naci|Naci|POL&Iacute;TICA|La naci|LA NACI|La Naci|Editorial|Opini|Sucesos|SUCESOS|OPINION|Educaci|EDUCACI|En Campa|EN CAMPA|Ambiente|AMBIENTE|Poder|Popular|Poder Popular|Política|Politica|Gesti|Social|Gestión|Regiones|Region/';
