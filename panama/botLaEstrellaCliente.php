...
set_time_limit (90);
    ...
    $URL = 'http://laestrella.com.pa/';
    $BOT_BROWSER_COOKIES = dirname(__FILE__).'/BotLaEstrella.txt';
    for ($w=1; $w<7; $w++)
    {
        if ($w > 5)
            exit ('No pude cargar la URL : ' . $URL . ' puede ser un cambio en el diseño o errores de servidor ' . $htmlResultPage);
            
        $htmlResultPage = EjecutaCurl($URL, $agentBrowser, $BOT_BROWSER_COOKIES, 'http://laestrella.com.pa/');
        
        $pattern = '/\<li\>\<h5\>Últimas noticias\<\/h5\>\<\/li\>(.*?)\<div class\=\"widget widget\-multimedia\"\>/si';
    ...
    foreach ($urlNoticias[0] as $k=>$urlNoticia) {
        if ($cont > $max)
            break;
        $urlNoticia = 'http://laestrella.com.pa' . $urlNoticia;
    ...
    function agregaNoticia($urlNoticia, $categoriaNoticia, $agentBrowser, $BOT_BROWSER_COOKIES, $URL)
    {
        ...
        $patron = '/\<div class\=\"post\-body\-content\"\>(.*?)\<ul class\=\"list\-inline post\-tags\"\>/si';
        ...
        $pattern = '/\<div class\=\"heading\-level\-1\"\>(.*?)\<div class\=\"post-body\"\>/si';
        preg_match($pattern, $htmlResultPage, $imagenes);
        
        if($imagenes){  
            $pattern = '/(?<=\<img class\=\"img\-responsive\" src\=\")[^\"]+/';
    ...
    {
        $patron = '/NACIÓN|NACION|POL&Iacute;TICA|La naci|LA NACI|La Naci|Editorial|Opini|Sucesos|SUCESOS|OPINION|Educaci|EDUCACI|En Campa|EN CAMPA|Ambiente|AMBIENTE|Poder|Popular|Poder Popular|Política|Politica|Gesti|Social|Gestión|Regiones|Region|panama|Panama|PANAMA/';
