...
set_time_limit (90);
    ...
    $URL = 'http://critica.com.pa/';
    $BOT_BROWSER_COOKIES = dirname(__FILE__).'/BotCritica.txt';
    for ($w=1; $w<7; $w++)
    {
        if ($w > 5)
            exit ('No pude cargar la URL : ' . $URL . ' puede ser un cambio en el diseño o errores de servidor ' . $htmlResultPage);
            
        $htmlResultPage = EjecutaCurl($URL, $agentBrowser, $BOT_BROWSER_COOKIES, 'http://critica.com.pa/');
        
        $pattern = '/\<a id\=\"main-content\"\>\<\/a\>(.*?)\<section class\=\"block block\-block cortinilla block\-block\-8\"\>/si';
    ...
    $patron = '/(?<=\<h2 class\=\"title\_news\_home\"\><a href\=\")[^\"]+/si';
    ...
    foreach ($urlNoticias[0] as $k=>$urlNoticia) {
        if ($cont > $max)
            break;
        $urlNoticia = 'http://critica.com.pa' . $urlNoticia;
    ...
    function agregaNoticia($urlNoticia, $categoriaNoticia, $agentBrowser, $BOT_BROWSER_COOKIES, $URL)
    {
        ...
        $patron = '/\<div class\=\"field field\-name\-body(.*?)\<a class\=\"a2a\_button\_twitter\"\>\<\/a\>/si';
        ...
        $pattern = '/\<div  id\=\"flexslider\-1\"(.*?)\<\/ul\>\<\/div\>/si';
    ...
    {
        $patron = '/NACIÓN|NACION|naci|Naci|POL&Iacute;TICA|La naci|LA NACI|La Naci|Editorial|Opini|Sucesos|SUCESOS|sucesos|OPINION|Educaci|EDUCACI|En Campa|EN CAMPA|Ambiente|AMBIENTE|Poder|Popular|Poder Popular|Política|Politica|Gesti|Social|Gestión|Regiones|Region|panama|Panama|PANAMA/';
