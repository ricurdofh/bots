...
set_time_limit (90);
    ...
    $URL = 'http://www.midiario.com/';
    $BOT_BROWSER_COOKIES = dirname(__FILE__).'/BotMiDiario.txt';
    for ($w=1; $w<7; $w++)
    {
        if ($w > 5)
            exit ('No pude cargar la URL : ' . $URL . ' puede ser un cambio en el diseño o errores de servidor ' . $htmlResultPage);
            
        $htmlResultPage = EjecutaCurl($URL, $agentBrowser, $BOT_BROWSER_COOKIES, 'http://www.midiario.com/');
        
        $pattern = '/\<div class\=\"highlighted\"\>(.*?)\<\/div\>\<\/div\>\<\/div\>/si';
    ...
    foreach ($urlNoticias[0] as $k=>$urlNoticia) {
        if ($cont > $max)
            break;
        $urlNoticia = 'http://www.midiario.com' . $urlNoticia;
    ...
    function agregaNoticia($urlNoticia, $categoriaNoticia, $agentBrowser, $BOT_BROWSER_COOKIES, $URL)
    {
        ...
        $patron = '/\<div class\=\"main\_body\"\>(.*?)\<style\>/si';
        ...
        $patron = '/<div class\=\"main\_sec\_title\"\>(.*?)\<\/div\>/si';
        ...
        $tituloNoticia = preg_replace('/\| Mi Diario .*/', '', trim(htmlspecialchars_decode($tituloNoticia[0])));
        ...
        $pattern = '/\<div class\=\"field\-content\"\>(.*?)\<\/div\>/si';
