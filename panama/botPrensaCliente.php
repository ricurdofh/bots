...
set_time_limit (90);
    ...
    $URL = 'http://www.prensa.com/';
    $BOT_BROWSER_COOKIES = dirname(__FILE__).'/BotPrensa.txt';
    for ($w=1; $w<7; $w++)
    {
        if ($w > 5)
            exit ('No pude cargar la URL : ' . $URL . ' puede ser un cambio en el diseño o errores de servidor ' . $htmlResultPage);
            
        $htmlResultPage = EjecutaCurl($URL, $agentBrowser, $BOT_BROWSER_COOKIES, 'http://www.prensa.com/');
        
        $pattern = '/\<div id\=\"pane\-2\_topmost\_(.*?)\<\/div\>/si';
    ...
    foreach ($urlNoticias[0] as $k=>$urlNoticia) {
        ...
        if(!preg_match('/http\:\/\//', $urlNoticia)){
            $urlNoticia = 'http://www.prensa.com' . $urlNoticia;
        }
    ...
    function agregaNoticia($urlNoticia, $categoriaNoticia, $agentBrowser, $BOT_BROWSER_COOKIES, $URL)
    {
        ...
        $patron = '/\<div class\=\"mce\-body\"\>(.*?)\<\/div\>\<\!\-\- \.mce\-body \-\-\>/si';
        preg_match($patron, $htmlResultPage, $contenidoNoticia);
        $contenidoNoticia = preg_replace('/\<div class\=\"inset\-mce\-relinfo\"\>(.*?)\<\/div\>\<\!\-\- \.inset\-mce\-relinfo \-\-\>/', '', $contenidoNoticia[1]);
        $contenidoNoticia = preg_replace('/\<figure class="mg">(.*?)\<\/figure\>\<\!\-\- \.mg \-\-\>/', '', $contenidoNoticia);
        
        $fechaNoticia = date("Y-m-d");
        $horaNoticia = date("h:i A");
        $horaNoticiaGmt = date("H:i:s");
        
        $patron = '/\<span class\=\"tags\"\>(.*?)\<\/span\>\<\!\-\- \.tags \-\-\>/si';
        ...
        $tituloNoticia = trim(htmlspecialchars_decode($tituloNoticia[0]));
        // $tituloNoticia = str_replace('| ', '', trim(htmlspecialchars_decode($tituloNoticia[0])));
        ...
        $contenidoNoticia = strip_tags($contenidoNoticia, '<br></br></ br><iframe></iframe><p></p><object></object><param></param>');
        ...
        $pattern = '/\<link rel\=\"image\_src\"(.*?)\" \/\>/si';
    ...
    {
        $patron = '/NACIÓN|NACION|naci|Naci|POL&Iacute;TICA|La naci|LA NACI|La Naci|Editorial|Opini|Sucesos|SUCESOS|OPINION|Educaci|EDUCACI|En Campa|EN CAMPA|Ambiente|AMBIENTE|Poder|Popular|Poder Popular|Política|Politica|politica|política|Gesti|Social|Gestión|Regiones|Region/';
        ...
        else 
        {
            ...
            {
                $patron = '/EUROPA|internacional|mundo|Mundo|MUNDO|vaticano/';
                preg_match($patron, $stringCategoria, $categoria);
                if($categoria)
                    $categoria = '4';
                else
